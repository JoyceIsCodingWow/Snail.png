...
class test_commands(commands.Cog):
	...
	@commands.command()
	@commands.is_owner()
	async def oldsetup(self, ctx):
		embedSetup = discord.Embed(color=0x0000FF)
		embedSetup.set_author(name=bot.user, icon_url=bot.user.avatar_url)
		embedSetup.add_field(name=':zero: Set mod-log channel', value='All moderation related action will be sent to this channel', inline=True)
		embedSetup.add_field(name=':one: Set message-log channel', value='All message updates will be sent to this channel', inline=True)
		embedSetup.add_field(name=':heavy_check_mark: Finish setup', value='Use the reaction whenever you are done setting up')
		msg = await ctx.send(embed=embedSetup)
